...
export class QuestionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.question = `What colour is the
    Incredible Hulk on his very
    first comic book cover?`//this.ocrText.substring(questionStartIdx, questionEndIdx);
    this.answers = ["Green", "Grey", "Red"];//this.ocrText.substring(questionEndIdx + 1, answersEndIdx - 1).split(/\n/);
    ...
    .map(
      (res: any) => {
        if (res.json().items)
        {
          ...
          self.snippets[i] = self.snippets[i].replace(regExp, `<span class="match">$&</span>`);
